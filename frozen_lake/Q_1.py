	...
	def find_batch_to_train(self, sess, m):
		...
		for j in range(batches_len):
			...
			for i in range(batch_len):
				...
				pp = sess.run(m.A2, feed_dict = {m.input_s:np.identity(self.n_state)[state:state+1]})
		...
		#print('x,y',x,y)
		for j in range(batches_len):
			sess.run(m.updateModel,feed_dict = {m.input_s:x[j], m.nextQ:y[j]})
...
if __name__ == "__main__":
	...
	with tf.Session() as sess:
		...
		for i in range(n_state):
			print np.argmax(sess.run(model_.A2, feed_dict = {model_.input_s:np.identity(n_state)[i:i+1]}))
